...
const Contact = () => {
  ...
  //New onSubmit function to handle form submission

  const onSubmit = async (data) => {
    setIsSubmitting(true);
    try {
      // Combine first and last name into a single 'name' field for the backend
      const payload = {
        name: `${data.firstName} ${data.lastName}`,
        email: data.email,
        message: data.message,
        // The backend isn't expecting 'subject' or 'phone', so we can optionally send them
