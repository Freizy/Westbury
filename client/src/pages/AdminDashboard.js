...
const AdminDashboard = () => {
  ...
  // Fetch dashboard data new code added
  const fetchDashboardData = async () => {
    try {
      ...
      // Process mandatory responses
      const stats = await responses[0].json();
      const attendance = await responses[1].json();
      const inventoryData = await responses[2].json();
      const bookingsData = await responses[3].json();
      const suitesData = await responses[4].json();
      ...
      // Set superadmin-specific data with corrected indices
      if (adminUser?.role === "superadmin") {
        const staffData = await responses[5].json(); // Corrected index
        const adminsData = await responses[6].json(); // Corrected index
      ...
      setLoading(false);
      // Set all states to empty on error to prevent render crashes
      setDashboardStats({});
      setAttendanceLogs([]);
      setInventory([]);
      setBookings([]);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Set staff and admin data if superadmin
      if (adminUser?.role === "superadmin") {
        const staff = await responses[4].json();
        const admins = await responses[5].json();
      ...
      <div className="bg-[#161616] text-white p-6">
        <div className="container mx-auto flex justify-between items-center">
          <div className="flex items-center space-x-4">
            ...
            />
            <div>
              <h1 className="text-2xl font-garamond font-bold mb-2">
                Admin Portal
              </h1>
      ...
      {process.env.NODE_ENV === "development" && (
        <div className="container mx-auto px-6 py-2">
          <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-2 rounded text-sm">
            ...
            {localStorage.getItem("adminToken") ? "Present" : "Missing"} | User:{" "}
            {adminUser?.email || "None"} | Role: {adminUser?.role || "None"} |
            Bookings: {bookings.length} | Active Tab: {activeTab}
          </div>
        </div>
      )}
        ...
        {activeTab === "dashboard" && (
          ...
          >
              ...
              <div className="bg-white rounded-lg p-6 shadow-sm border">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-garamond font-bold text-[#161616]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-lg p-6 shadow-sm border">
                <h3 className="text-xl font-garamond font-bold text-[#161616] mb-4">
                  Recent Attendance
                </h3>
                <div className="space-y-3">
                  {attendanceLogs &&
                    attendanceLogs.slice(0, 5).map((log) => (
                      ...
                      >
                        <div>
                          <p className="font-sans font-medium text-[#161616]">
                            {log.staffName}
                          </p>
                          <p className="text-sm text-[#838FBF]">
                            {log.date} • {log.timeIn}
                          </p>
                        </div>
                        <span
                    ...
                    ? inventory
                        ...
                        .map((item) => (
                          ...
                          >
                            ...
                            <span className="px-2 py-1 rounded-full text-xs font-sans bg-red-100 text-red-800">
                              Low Stock
                            </span>
                          </div>
                        ))
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-sans font-medium text-[#838FBF] uppercase tracking-wider">
                        Name
                      </th>
                      <th className="px-6 py-3 text-left text-xs font-sans font-medium text-[#838FBF] uppercase tracking-wider">
                        Position
        ...
        {activeTab === "availability" && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {suites.map((suite) => {
                const suiteBookings = bookings.filter(
                  (b) => b.property_id === suite.id
                );
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-lg font-garamond font-semibold text-[#161616]">
                        {suite.title}
                      </h3>
                      <span
                    ...
                    {suiteBookings.length > 0 && (
                      <div>
                        <h4 className="text-sm font-sans font-medium text-[#161616] mb-2">
                          Upcoming Bookings:
                        </h4>
                        <div className="space-y-1">
                          {suiteBookings
                            .filter(
                              (b) => new Date(b.check_in_date) >= new Date()
                            )
                            .slice(0, 3)
                            .map((booking) => (
                              <div
            ...
            <div className="bg-white rounded-lg p-6 shadow-sm border">
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-4 py-2 text-left text-xs font-sans font-medium text-[#838FBF] uppercase">
                        Suite
                      </th>
                      <th className="px-4 py-2 text-left text-xs font-sans font-medium text-[#838FBF] uppercase">
                        Current Status
                      ...
                      return (
                        <tr key={suite.id}>
                          <td className="px-4 py-3">
                            ...
                            <div className="text-xs text-[#838FBF]">
                              ${suite.price?.toLocaleString()}/night
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            {currentBooking ? (
                              <div>
                                ...
                                <div className="text-xs text-[#838FBF] mt-1">
                                  Until{" "}
                                  {new Date(
                              ...
                              <span className="px-2 py-1 rounded-full text-xs font-sans bg-green-100 text-green-800">
                                Available
                              </span>
                            )}
                          </td>
                          <td className="px-4 py-3">
                            {nextAvailable ? (
                              <div className="text-sm text-[#838FBF]">
                                {new Date(
                                  nextAvailable.check_out_date
                                ).toLocaleDateString()}
                              </div>
                            ) : (
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {editingItem === item.id ? (
                            <input
                              ...
                              onBlur={(e) =>
                                handleUpdateInventory(item.id, e.target.value)
                              }
                              onKeyPress={(e) =>
                                e.key === "Enter" &&
                                handleUpdateInventory(item.id, e.target.value)
                    ...
                    {bookings.map((booking) => {
                      return (
                        <tr key={booking.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-sans text-[#838FBF]">
                              {suites.find((s) => s.id === booking.property_id)
                                ?.title || `Suite #${booking.property_id}`}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-sans text-[#838FBF]">
                              ...
                              {booking.status === "pending" && (
                                <>
                                  <button
                                    onClick={() => {
                                      console.log(
                                        "Confirm button clicked for booking:",
                                        booking.id
                                      );
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleAddStaff} className="space-y-4">
              <div>
                <label className="block text-sm font-sans font-medium text-[#161616] mb-1">
                  Name
                </label>
                <input
                  type="text"
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#710014]"
                />
              </div>
              <div>
                <label className="block text-sm font-sans font-medium text-[#161616] mb-1">
              ...
              <div className="flex space-x-3 pt-4">
                <button
                  type="submit"
                  className="flex-1 bg-[#710014] hover:bg-[#161616] text-white py-2 rounded-lg font-sans font-medium transition-colors"
                >
                  Add Staff
                </button>
                <button
                  ...
                  className="flex-1 bg-gray-300 hover:bg-gray-400 text-[#161616] py-2 rounded-lg font-sans font-medium transition-colors"
                >
                  Cancel
                </button>
              </div>
