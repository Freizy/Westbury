...
const Suites = () => {
  ...
  const fetchSuites = useCallback(async () => {
    try {
      ...
      // The server returns { success: true, data: properties }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                  <div>
                    <select
                      ...
                      className="w-full px-4 py-3 rounded-lg bg-white/20 border border-white/30 text-black focus:outline-none focus:ring-2 focus:ring-[#838FBF] font-sans"
                    >
        ...
        <section className="py-16">
          <div className="container mx-auto px-4">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
              <div>
                <h2 className="text-3xl font-serif font-bold text-[#161616] mb-2">
                  {loading ? 'Loading...' : `${Array.isArray(suites) ? suites.length : 0} Suites Available`}
                </h2>
            ...
            {!loading && Array.isArray(suites) && suites.length > 0 && (
              ...
              >
                {suites
                  .filter(suite => suite && suite.id) // Filter out invalid suites
                  .map((suite, index) => (
                    <motion.div
                      key={suite.id}
                      initial={{ opacity: 0, y: 30 }}
            ...
            {/* Load More Button */}
            {!loading && Array.isArray(suites) && suites.length > 0 && suites.length >= 9 && (
