...
const PropertyDetail = () => {
  ...
  const images = Array.isArray(property.images) && property.images.length > 0 ? property.images : [];
      let galleryImages = images.filter(img => img && !img.startsWith('client/src/Images') && !img.startsWith('./') && !img.startsWith('../'));
    
    // Handle labeled images from IMAGES folder
    const labeledImages = images.filter(img => img && img.startsWith('client/src/Images/IMAGES/'));
    if (labeledImages.length > 0) {
      galleryImages = labeledImages.map(img => {
        const imageName = img.split('/').pop();
        return require(`../Images/IMAGES/${imageName}`);
      });
    }
    
    if (galleryImages.length === 0) {
      galleryImages = [suiteImageMap[property.title] || require('../Images/IMAGES/KARR.JPG')];
    }
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6">
            ...
            <div className="mt-6 lg:mt-0">
              <div className="text-4xl font-serif font-bold text-[#710014] mb-2">
                <span className="text-2xl font-garamond font-bold text-[#710014]">${property.price?.toLocaleString()}</span>
            ...
            >
              ...
              <div className="space-y-4">
                <div className="flex items-center text-[#838FBF] font-sans">
                  ...
                  <a href="tel:+233201234567" className="hover:text-[#710014] transition-colors">
                    +233 20 123 4567
                  </a>
                </div>
                <div className="flex items-center text-[#838FBF] font-sans">
                  <Mail size={20} className="mr-3 text-[#710014]" />
                  <a href="mailto:info@luxuryaccra.com" className="hover:text-[#710014] transition-colors">
                    info@luxuryaccra.com
                  </a>
                </div>
              </div>
              <Link
