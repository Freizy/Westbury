...
router.get('/auth/admins', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
    const admins = await query('SELECT id, username, email, role, created_at FROM users WHERE role IN ("admin", "superadmin") ORDER BY created_at DESC');
    res.json({
      success: true,
      data: admins
    });
...
// Public staff list (for staff portal)
router.get('/staff', async (req, res) => {
  try {
    const staff = await query('SELECT id, name, position FROM staff WHERE status = "active" ORDER BY name');
    res.json({
      success: true,
      data: staff
    });
...
// Add staff (admin route - same as /staff but for consistency)
router.post('/staff/admin', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
    const { name, position, email, phone } = req.body;
...
// Attendance routes (admin and superadmin)
...
router.post('/attendance/login', async (req, res) => {
  try {
    ...
    // Check if staff already logged in today
...
// Inventory routes (admin and superadmin)
