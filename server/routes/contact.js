...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await run(
      'INSERT INTO contact_inquiries (name, email, phone, message, property_id) VALUES (?, ?, ?, ?, ?)',
...
router.get('/', async (req, res) => {
  try {
    const inquiries = await query(
      'SELECT ci.*, p.title as property_title FROM contact_inquiries ci LEFT JOIN properties p ON ci.property_id = p.id ORDER BY ci.created_at DESC'
    );
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const inquiries = await query(
      'SELECT ci.*, p.title as property_title FROM contact_inquiries ci LEFT JOIN properties p ON ci.property_id = p.id WHERE ci.id = ?',
      [id]
    );
