...
router.post('/', async (req, res) => {
  try {
    ...
    // Validation
    if (!property_id || !guest_name || !guest_email || !guest_phone || 
    ...
    const result = await run(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [property_id, guest_name, guest_email, guest_phone, check_in_date, check_out_date, guests_count, total_amount, special_requests || null]);
    
    console.log('Booking inserted successfully with ID:', result.id);
      ...
      try {
        ...
        const mailOptions = {
          ...
          html: `
            ...
            <p>Best regards,<br>Luxury Apartments Accra Team</p>
          `
        };
